...
prob.vol_pars_fcn = @(x)  [(365*(x(1)<=xlims(2)/2)+36.5*(x(1)>xlims(2)/2)); ...
    (188*(x(1)<=xlims(2)/2)+18.8*(x(1)>xlims(2)/2)); 0;0]; % TODO
prob.bnd_pars_fcn = @(x,bnd) (bnd==2)*[0;0]+(bnd==3)*[0;0.1]+(bnd==4)*[0;0];  % TODO
...
dbc_idx = get_gdof_from_bndtag([1 2],1,2,ldof2gdof,e2bnd,f2v);% TODO
dbc_val = 0*dbc_idx;% TODO
...
if pltit
    ...
    xeval =[linspace(xlims(1),xlims(2),nel(1));ones(1,nel(1))*0.5]; % TODO
